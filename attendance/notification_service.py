...
def send_reminders_to_unchecked(attendance, token):
    """
    Send notifications to enrolled students who have NOT yet checked in.
    """
    course = attendance.course
    present_ids = set(attendance.present_students.values_list('id', flat=True))
    all_students = course.students.select_related('user').all()
    
    unchecked_students = [student for student in all_students if student.id not in present_ids]
    emails_to_send = []
    
    for student in unchecked_students:
        if student.should_send_email_notifications():
            subject = f"Reminder: Live Attendance Session - {course.name}"
            context = {
                'student': student,
                'course': course,
                'token': token,
            ...
            html_message = render_to_string('emails/attendance_started.html', context)
            plain_message = f"Reminder: An attendance session is currently active for {course.name} ({course.course_code}). Please check in immediately. Token: {token}"
    ...
    if emails_to_send:
        try:
            ...
            logger.error("Failed to send reminder emails: %s", e)