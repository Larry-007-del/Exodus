...
class SubmitLocationView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        logger.info(
            ...
            "lecturer_lat=%s lecturer_lon=%s radius=%s effective_radius=%.0f ble_verified=%s",
            request.user.username, lat_float, lon_float, accuracy,
            attendance.lecturer_latitude, attendance.lecturer_longitude,
            attendance.radius_meters, effective_radius, ble_verified
        )

        is_valid_location = ble_verified or attendance.is_within_radius(lat_float, lon_float, radius_meters=effective_radius)

        if is_valid_location:
            # If BLE was used, hardcode their coordinates to exactly match the lecturer to force 0m distance in history logs
            if ble_verified:
